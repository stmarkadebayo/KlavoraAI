...
from klavora_ai.demo_profiles import CONTRACT_DEMO_DEFAULTS, build_demo_profile  # noqa: E402
...
def _resolve_contract_output_dir(profile: str, explicit_output_dir: str | None) -> Path:
    if explicit_output_dir:
        return Path(explicit_output_dir)
    if profile == "demo":
        return ROOT / "data" / "demo" / "contract"
    return ROOT / "data" / "processed" / "contract_main"
...
def main() -> int:
    ...
    output_dir = _resolve_contract_output_dir(args.profile, args.output_dir)

    profile_report = None
    if args.profile == "demo":
        profile_defaults = CONTRACT_DEMO_DEFAULTS
    ...
    counts = write_contract_processed_splits(examples, output_dir, quality_report=final_quality_report)
    print(
        json.dumps(
            {
